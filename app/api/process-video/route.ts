...
export async function POST(request: NextRequest) {
  try {
    ...
    const basketDetections = await analyzeFramesForBaskets(frames)
    console.log(`Found ${basketDetections.length} potential basket events`)

    // Step 4: Determine successful shots and create clips
    const shots = await determineShotsAndClips(basketDetections, metadata)
...
async function extractFrames(videoUrl: string, fps: number) {
...
async function analyzeFramesForBaskets(frames: any[]) {
  console.log("Analyzing frames for basketball detection...")
...
async function simulateBasketDetection(frames: any[]) {
  ...
  const { object: detections } = await generateObject({
    model: openai("gpt-4o"),
    schema: z.object({
      detections: z.array(
        z.object({
    ...
    prompt: `Analyze these ${frames.length} basketball video frames and detect:
...
async function determineShotsAndClips(detections: any[], metadata: any) {
  ...
  for (let i = 0; i < shotSequences.length; i++) {
    ...
    const { object: shotAnalysis } = await generateObject({
      model: openai("gpt-4o"),
      schema: z.object({
        made: z.boolean(),
      ...
      prompt: `Analyze this basketball shot sequence at timestamp ${shotTimestamp}s:
